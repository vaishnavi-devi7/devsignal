...
db.query = async (text, params) => {
  ...
  if (text.includes('SELECT id FROM dsa_problems')) {
    ...
    if (params[2] === '[999A] Duplicate Problem') {
      return { rows: [{ id: 10 }] };
    }
    return { rows: [] };
  }
...
axios.get = async (url) => {
  if (url.includes('invalid_handle')) {
    const error = new Error();
    error.response = { status: 400, data: { status: 'FAILED' } };
    throw error;
  }
...
async function run() {
  const req = { user: { id: 99 }, body: { username: 'valid_handle' } };
  let status, response;
  const res = {
  ...
  // Test invalid handle
  const req2 = { user: { id: 99 }, body: { username: 'invalid_handle' } };
  await syncCodeforces(req2, res);
  console.log("Invalid Handle Response:", status, response.message);
}
